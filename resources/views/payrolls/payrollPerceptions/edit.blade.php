        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                <div class="relative overflow-x-auto shadow-md sm:rounded-lg px-4 py-4">
                    ...
                    <form method="POST" action="{{ route('payroll_perceptions.update', $payroll_perception->payroll_perception_id) }}">
                        @csrf
                        @method('PUT')
                        ...
                        <div>
                            <x-label for="perception_id" value="{{ __('Perception') }}" />
                            <select name="perception_id" id="perception_id" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-white-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-black dark:focus:ring-blue-500 dark:focus:border-blue-500">
                                <option value="{{ $p_perceptions[0]->perception_id }}" selected>{{ $p_perceptions[0]->name }}</option>
                                @foreach ($perceptions as $perception)
                                    <option value="{{ $perception->perception_id }}">{{ $perception->name }}</option>
                                @endforeach
                            </select>
                        </div> <br>
                        ...
                        <div>
                            ...
                            <x-input id="sum" class="block mt-1 w-full" type="number" step="0.01" name="sum" value="{{ $payroll_perception->sum }}" required autofocus autocomplete="sum" />
                        </div> <br>
