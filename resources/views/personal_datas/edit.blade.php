        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                <div class="relative overflow-x-auto shadow-md sm:rounded-lg px-4 py-4">
                    ...
                    <form method="POST" action="{{ route('personal_datas.update', $personal_data) }}">
                        @csrf
                        @method('PUT')
                        ...
                        <div>
                            ...
                            @foreach ($employees as $employee)
                                @if($personal_data->employee_id == $employee->employee_id)
                                    <x-input class="block mt-1 w-full" type="text" name="employee_name" value="{{$employee->name}}  {{$employee->last_name1}} {{$employee->last_name2}}" disabled required autofocus autocomplete="employee_id" />
                                @endif
                            @endforeach
                        </div>
